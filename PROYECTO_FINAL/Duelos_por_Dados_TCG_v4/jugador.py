"""jugador.py
Modelo de Jugador para el TCG de dados.
"""
import random
...
class Jugador:
    def __init__(self, nombre: str, vida_maxima: int = VIDA_INICIAL, cartas=None):
        self.nombre = nombre
    ...
    def iniciar_turno(self, modo_avanzado: bool):
        if modo_avanzado:
        ...
        cantidad = DADOS_INICIALES - (self.perder_dados_siguiente_turno if modo_avanzado else 0)
        if cantidad < 1:
            cantidad = 1
        if modo_avanzado:
            self.perder_dados_siguiente_turno = 0
    ...
    def finalizar_turno(self, modo_avanzado: bool):
        if modo_avanzado and self.rerolls_realizados > 0:
            self.bloquear_reroll_proximo_turno = True

    def hacer_reroll(self, modo_avanzado: bool):
        ...
        for _ in range(len(self.dados)):
            ...
            valor = randint(1, 6) + self.rerolls_realizados
            if valor > 8:
                valor = 8
            nuevos.append(Dado(valor))
        self.dados = nuevos
        self.perder_dados_siguiente_turno = min(self.rerolls_realizados, 2)
        return True

    def usar_dado(self, indice: int):
